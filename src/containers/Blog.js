...
import blogIcon from '../assets/img/hero/blog.svg';
...
const Blog = () => {
  ...
  return isLoading ? (
    ...
    <div className="blog">
    <HeroSection title={"Blog"} img={blogIcon} topTitle={"Read up on the latest news"}/>
   <div className="blog__container">
     <div className="mobile-navigator">
       <button className="open-more"  onClick={handleToggle}> <i class="fa fa-filter"></i> Open</button>
     </div>
       ...
       <div className="blog-page">
         ...
         {data.slice(0, 5).map((data) => {
            return (
             ...
             <div className="main-content-div" onClick={() => history.push(`/article/${data._id}`)}>
               <div className="img-cont" style={{backgroundImage: `url(${data.picture})`}}></div>
                 <h1>{data.title}</h1>
                 <p className="main-content-p" dangerouslySetInnerHTML={{ __html: data.text }}></p>
             </div>
             <div className="under-main-div">
               <div className="mobile-grid">
               <div className="date-under left"><i className="fa fa-calendar"></i><span>20/09/2022</span></div>
               <div className="seperator-dot"><i className="fa fa-circle"></i></div>
               <div className="date-under right"><i className="fa fa-user"></i><span>{data.author}</span></div></div>
               <div className="like-counter"><i className="far fa-heart"></i><span>65</span></div>
             </div>
           </div> 
            );
          })}
         ...
         <div className={menuOn ? "sidebar" : "sidebar-on"}>
           <div className="close-btn"  onClick={handleToggle}><i
            className="fas fa-times icn-x"></i></div>
           <div className="blog-content">
               <h3>About Us</h3>
               <p>We are ITRSQ , An all-rounder IT solutions company, based in Londo providing IT Support and Web Development Services. We have a team of proactive individuals willing to go that extra mile to ensure your needs are met. We believe in value, trust, and transparency.</p>
               ...
               {data.slice(5, 9).map((data) => {
                  return (
                    <div className="single-item" onClick={() => history.push(`/article/${data._id}`)}>
                    <div className="img-holder">
                       <img src={data.picture} alt={data.picture}/>
                    </div>
                    <div className="inner-grid">
                      <div className="title-single-line"><span>{data.title}</span></div>
                        <div className="under-title-single-line">
                          <div className="date"><i className="fa fa-calendar"></i><span>20/09/2022</span> </div>
                          <div className="seperator-dot"><i className="fa fa-circle"></i></div>
                          <div className="likes"><i className="fa fa-heart"></i><span>64</span> 
                          </div>
                        </div>
                      </div>
                  </div>
